...
public interface CategoriaRepository extends JpaRepository<CategoriaEntity, UUID> {
	...
	@Query("SELECT c FROM CategoriaEntity c WHERE c.usuarioId IS NULL OR c.usuarioId = ?1 ORDER BY c.tipo ASC, c.nome ASC")
	...
	@Query("SELECT COUNT (*) > 0 FROM CategoriaEntity c WHERE c.nome = ?1 AND c.tipo = ?2 AND c.usuarioId = ?3")
	boolean existePor(String nome, TipoCategoriaEnum tipo, UUID usuarioId);
